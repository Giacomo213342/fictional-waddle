...
struct _PolyculeClient {
  GtkApplication parent_instance;
  char** dart_entrypoint_arguments;
};

G_DEFINE_TYPE(PolyculeClient, polycule_client, GTK_TYPE_APPLICATION)
...
static void polycule_client_activate(GApplication* application) {
  ...
  if (use_header_bar) {
    ...
    gtk_header_bar_set_title(header_bar, "< polycule >");
    gtk_header_bar_set_show_close_button(header_bar, TRUE);
    gtk_window_set_titlebar(window, GTK_WIDGET(header_bar));
  } else {
    gtk_window_set_title(window, "< polycule >");
  }
...
static void polycule_client_dispose(GObject* object) {
  PolyculeClient* self = POLYCULE_CLIENT(object);
  g_clear_pointer(&self->dart_entrypoint_arguments, g_strfreev);
  G_OBJECT_CLASS(polycule_client_parent_class)->dispose(object);
}
