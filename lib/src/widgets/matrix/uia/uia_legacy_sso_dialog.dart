...
class UiaLegacySSODialog extends StatefulWidget {
  const UiaLegacySSODialog({
    super.key,
  ...
  @override
  State<UiaLegacySSODialog> createState() => _UiaLegacySSODialogState();
}

class _UiaLegacySSODialogState extends State<UiaLegacySSODialog> {
  bool _loading = false;
          ...
          children: [
            ...
            Padding(
              ...
              child: FloatingActionButton.extended(
                ...
                onPressed: _loading ? null : _fallbackDeeplink,
                icon: _loading
  ...
  Future<void> _fallbackDeeplink() async {
    setState(() {
      _loading = true;
    });
    ...
    try {
      ...
      final uri = client.homeserver?.resolveUri(
        Uri.parse(
          '/_matrix/client/v3/auth/${AuthenticationTypes.sso}/fallback/web?session=${client.deviceID}',
        ),
      );
      if (uri == null) {
        return;
