...
SPEC_VERSION="$(grep 'static const Set<String> supportedVersions' "${MATRIX_ROOT}/lib/src/client.dart" | awk -F '=' '{ print $2 }' | awk -F ';' '{ print $1}' | sed "s/'/\"/g" | sed 's/{/[/g' | sed 's/}/]/g' | jq -cr '. | last' | sed 's/v//')"
